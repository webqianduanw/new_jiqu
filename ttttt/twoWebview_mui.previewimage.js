...
var nativeScrollOffsetY = 0;
var preWidth = window.innerWidth;
var preHeight = window.innerHeight;
var tempvar;
function nativeScrollViewSizeAndOffset(paramStr) {

	var offsetT = JSON.parse(paramStr);
	nativeScrollOffsetY = offsetT.y+'px';
	preWidth = offsetT.width+'px';
	preHeight = offsetT.height+'px';
	var pre = mui.getPreviewImage();
	pre.open(tempvar);
}

function getNativeScrollViewFrameAndOffset() {

	if (isiOS) {
		window.webkit.messageHandlers.getOffsetAndSize.postMessage(null);
	} else {
		window.android.getOffsetAndSize(null);
	}
}
...
(function ($, window) {
	...
	proto.initEvent = function () {
		...
		$(document.body).on('tap', 'img[data-preview-src]', function () {
			tempvar = this;
			getNativeScrollViewFrameAndOffset();
			return false;
		});
		var laterClose = null;
	...
	proto.refresh = function (index, groupArray) {
		...
		this.element.style.height = preHeight;
		this.element.style.width = preWidth;
		this.element.style.top = nativeScrollOffsetY;
